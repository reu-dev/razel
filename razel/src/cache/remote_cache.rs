...
impl GrpcRemoteCache {
    ...
    pub async fn download_and_store_blobs(
        ...
        for file in &stream_files {
            let digest = file.digest.as_ref().unwrap().into();
    ...
    async fn batch_download(
        ...
        downloaded: &mut Vec<(BlobDigest, PathBuf)>,
    ) {
        match self
            .cas_client
            .clone()
...
mod tests {
    ...
    async fn grpc_server_cas_bytestream() {
        ...
        let digest = BlobDigest::for_bytes(&data);
        ...
        assert_eq!(BlobDigest::for_bytes(&got), digest);
        assert_eq!(got, data);
    }
