...
import { Razel } from "https://deno.land/x/razel@v0.5.2/razel.ts";
...
razel.addTask("filtered.csv", "csv-filter", [
    "-i",
    c,
    "-o",
    razel.addOutputFile("filtered.csv"),
    "-c",
    "a",
    "xyz",
])
...
razel.addCommand("cmake-touch-files", "cmake", [
    "-E",
    "touch",
    "razel-out/cmake-touch-1",
    "razel-out/cmake-touch-2",
])
...
if (false) { // requires clang
    ...
    const say_hi = razel.addCommand("say_hi", "clang", [
        "-o",
        razel.addOutputFile("say_hi"),
        razel.addDataFile("data/say_hi.c"),
    ]);
