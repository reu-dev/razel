...
impl Project {
    /// First-time creation. The project storage path does not yet exist on disk.
    pub fn new(storage_root: &Path, id: AuthId) -> Result<Self> {
        let path = storage_root.join(&id);
        ...
        let job_db = JobDatabase::new(&path)?;
        Ok(Self {
            id,
            path,
            ac_dir,
            ...
            download_dir,
            cache_bytes: 0,
            locally_cached: HashSet::new(),
            requested_from_clients: Default::default(),
            job_db,
        })
    }

    /// Load existing on-disk state into this project at startup.
    ...
    pub fn bytes(&self) -> u64 {
        self.cache_bytes + self.job_db.bytes
    }

    /// Returns true if the file is locally cached, otherwise queues a download from the client.
    ...
    pub fn handle_request_file_finished(&mut self, digest: &Digest) -> Vec<(JobId, FileId)> {
        ...
        if self.locally_cached.insert(digest.hash.clone()) {
            self.cache_bytes += digest.size_bytes as u64;
        }
        requests
            .into_iter()
    ...
    pub fn handle_request_file_failed(
        ...
        if let Some(req) = requests.iter_mut().find(|r| r.connection.is_some()) {
            ...
            self.spawn_download_file(conn, self.id.clone(), job_id, file, tx.clone());
